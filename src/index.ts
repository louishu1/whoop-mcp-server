...
// --- Startup diagnostics (added to surface silent crashes on Render) ---
...
console.log('BOOT: opening database at', config.dbPath);
let db!: WhoopDatabase;
let client!: WhoopClient;
let sync!: WhoopSync;
try {
	db = new WhoopDatabase(config.dbPath);
	...
	sync = new WhoopSync(client, db);
	console.log('BOOT: init complete');
} catch (err) {
	console.error('!!! BOOT FAILED during init:', err);
	process.exit(1);
}
...
async function main(): Promise<void> {
	console.log('BOOT: main() starting, mode =', config.mode);
	if (config.mode === 'stdio') {
		...
		// OAuth 2.1 layer so Claude's connector handshake succeeds.
		mountOAuth(app);
		...
		console.log('BOOT: calling app.listen on port', config.port);
		const server = app.listen(config.port, '0.0.0.0', () => {
			console.log(`=== Whoop MCP server LISTENING on http://0.0.0.0:${config.port} ===`);
